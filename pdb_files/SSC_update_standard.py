...
def append_ssc_to_tsv(updated_tsv_filepath, ssc_filepath, output_filepath):
    """Appends SSC (Specificity Score) values to an existing TSV file based on a common RNA sequence.

    This function reads two data files: an updated TSV and an SSC dataset. It extracts
    'TargetSequence(RNAversion)' and 'SSC' from the SSC file, standardizes the
    'TargetSequence(RNAversion)' column in both dataframes (converts to uppercase
    and replaces 'U' with 'T'), and then performs a left merge. The final merged
    dataframe, including the SSC scores, is written to a new TSV file.

    Args:
        updated_tsv_filepath (str): Path to the input TSV file that needs SSC scores appended.
        ssc_filepath (str): Path to the SSC dataset file (e.g., a `.out` file, space-separated).
        output_filepath (str): Path where the final merged TSV file will be saved.

    Returns:
        None: The function writes the processed data to a file.
    """
