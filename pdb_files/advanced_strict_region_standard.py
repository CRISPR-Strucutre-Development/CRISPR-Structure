...
def process_advanced_metrics(pdb_dir, output_txt):
    """Processes PDB files to extract and calculate advanced entropy metrics for specific regions.

    This function iterates through PDB files in a given directory, extracts nucleotide
    sequences, and uses B-factor as an entropy metric. It then calculates mean, median,
    max, min, standard deviation, range, and interquartile range for predefined
    regions (TL, SL1, SL2, SL3) and writes these metrics along with the sequence
    to a tab-separated output file.

    Args:
        pdb_dir (str): The path to the directory containing PDB files.
        output_txt (str): The path to the output tab-separated file where results will be written.

    Returns:
        None: The function writes data directly to the specified output file.
    """
