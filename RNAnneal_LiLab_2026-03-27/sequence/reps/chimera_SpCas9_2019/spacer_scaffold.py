            ...
            for i in range(1, 6):
                ...
                if os.path.exists(pdb_path):
                    try:
                        ...
                        # Extract 1/0 for pairings using new regions
                        p_tl = check_region_interaction(pairings, res, spacer_region, regions["TL_32_37"])
                        p_sl1 = check_region_interaction(pairings, res, spacer_region, regions["SL1_54_60"])
                        p_sl2 = check_region_interaction(pairings, res, spacer_region, regions["SL2_71_76"])
                        p_sl3 = check_region_interaction(pairings, res, spacer_region, regions["SL3_88_90"])
                        
                        # Extract 1/0 for stackings using new regions
                        s_tl = check_region_interaction(stackings, res, spacer_region, regions["TL_32_37"])
                        s_sl1 = check_region_interaction(stackings, res, spacer_region, regions["SL1_54_60"])
                        s_sl2 = check_region_interaction(stackings, res, spacer_region, regions["SL2_71_76"])
                        s_sl3 = check_region_interaction(stackings, res, spacer_region, regions["SL3_88_90"])
                        
                        # Calculate continuous heavy atom minimum distance using new regions
                        d_tl = get_min_heavy_atom_distance(pdb_path, spacer_region, regions["TL_32_37"])
                        d_sl1 = get_min_heavy_atom_distance(pdb_path, spacer_region, regions["SL1_54_60"])
                        d_sl2 = get_min_heavy_atom_distance(pdb_path, spacer_region, regions["SL2_71_76"])
                        d_sl3 = get_min_heavy_atom_distance(pdb_path, spacer_region, regions["SL3_88_90"])
...
if __name__ == "__main__":
    target_directory = "/local/projects-t3/lilab/vmenon/SpCas9-Strucutre/RNAnneal_LiLab_2026-03-27/sequence/reps/chimera_SpCas9_2019/"
