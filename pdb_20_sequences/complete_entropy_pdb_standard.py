...
def process_cumulative_entropy_data(pdb_dir, out_file):
    ...
    the B-factor fields). Computes statistics including sum, median, max, min, 
    standard deviation, interquartile range (IQR), and range, and writes the 
    results to a TSV file.

    Args:
        pdb_dir (str): The directory containing the PDB files to process.
        out_file (str): The path to the output TSV file where statistics 
            will be written.
