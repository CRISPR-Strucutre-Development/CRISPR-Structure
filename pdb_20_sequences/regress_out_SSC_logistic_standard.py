...
def run_logistic_regression(filepath):
    """Runs logistic regression on structural metrics predicting Indel activity.

    Reads a tab-separated values dataset, binarizes the 'Indel' dependent 
    variable (1 if > 10%, 0 otherwise), and performs a series of logistic 
    regressions for various structural metrics while controlling for 'SSC'. 
    Features are standardized prior to fitting. Results, including log-odds 
    coefficients, odds ratios, and p-values, are sorted by p-value and printed 
    to standard output.

    Args:
        filepath (str): The path to the TSV file containing the dataset.
